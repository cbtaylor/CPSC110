...
(check-expect (render-cs (make-cs "Tim" (list "2" "1")))
              (above/align "left"
                           (rectangle (- WIDTH 20) (* TEXT-SIZE 2) "outline" "black")
                           (place-image/align (lot-to-image (list "2" "1"))
                                              TEXT-X TEXT-Y
                                              "left" "top"
                                              BACKGROUND)))
...
(define (render-cs cs)
  (above/align "left"
               (rectangle (- WIDTH 20) (* TEXT-SIZE 2) "outline" "black")
               (place-image/align (lot-to-image (cs-lot cs))
                                  TEXT-X TEXT-Y
                                  "left" "top"
                                  BACKGROUND)))
...
(check-expect (handle-key (make-cs "Tim" (list "9")) "7")   ;nothing happens
              (make-cs "Tim" (list "9")))
(check-expect (handle-key (make-cs "Tim" (list "a")) "b")
              (make-package (make-cs "Tim" (list "a")) (list "twt" "b")))

;; since handling return involves using the random function
;; we can't write a test for it in the normal way
